...
export const SoundCardOptions: React.FC<SoundCardOptionsProps> = ({
  ...
  return <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        const bahasaMelayuTranslation = getBahasaMelayuTranslation(option);
        return <Button key={option} onClick={() => onResponse(option)} className="bg-gray-100 hover:bg-[#005da9] hover:text-white text-gray-800 rounded-full px-6 py-4 text-sm transition-colors border border-gray-300 flex flex-col items-center min-h-[100px] w-full gap-0 justify-between
">
              <span className="leading-tight">{option}</span>
              {bahasaMelayuTranslation && <span className="text-xs md:text-sm mt-0.5 opacity-70 leading-tight">
                  {bahasaMelayuTranslation}
                </span>}
