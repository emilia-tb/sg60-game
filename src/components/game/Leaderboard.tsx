...
export const Leaderboard: React.FC<LeaderboardProps> = ({
  ...
  return <Card className="w-full bg-white shadow-lg border-0 rounded-3xl p-4">
      <CardContent className="space-y-4">
        <h3 className="sg-subheading text-xl text-center pt-4">Leaderboard</h3>
        {leaderboard.length > 0 ? <div className="space-y-2">
            {leaderboard.map((player, index) => <div key={`${player.name}-${player.timestamp}`} className={`flex items-center justify-between p-3 rounded-lg ${player.name === playerName ? 'bg-yellow-100 border-2 border-yellow-400' : 'bg-gray-50'}`}>
          ...
          </div> : <p className="sg-body text-center opacity-70">
