    ...
    <Card className="w-full bg-white shadow-lg border-0 rounded-3xl p-4 md:p-8">
      <CardContent className="space-y-6 md:space-y-8">
        <div className="text-center space-y-4">
          <h2 className="sg-subheading">Thanks for playing, {playerName}!</h2>
        ...
        <div className="space-y-4 p-4 md:p-6 bg-yellow-50 rounded-xl">
          ...
          <div className="flex justify-center space-x-2" onMouseLeave={() => setHoveredRating(0)}>
            {[1, 2, 3, 4, 5].map(star => (
              <button key={star} onClick={() => setRating(star)} onMouseEnter={() => setHoveredRating(star)} className={`text-3xl transition-colors ${star <= (hoveredRating || rating) ? 'text-yellow-500' : 'text-gray-300 hover:text-yellow-400'}`}>
                {star <= (hoveredRating || rating) ? '⭐' : '☆'}
              </button>
            ))}
          </div>
          {rating > 0 && <p className="sg-body text-center text-sm opacity-70">Thank you for your feedback! ({rating} star{rating !== 1 ? 's' : ''})</p>}
        </div>

        <div className="flex justify-center mt-6">
          <Button onClick={handleSubmit} disabled={!canSubmit} className="text-white px-8 py-3 bg-[#e40048] rounded-3xl font-bold text-base disabled:opacity-50">
            Continue to Results
          </Button>
        </div>
      </CardContent>
